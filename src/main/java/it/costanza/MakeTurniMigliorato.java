...
public class MakeTurniMigliorato {
    ...
    public static void main(String[] args) throws IOException, ExceptionCustom {
        ...
        ArrayList<SuperRun> listaRun = new ArrayList<>();
        ...
        for (int i = 0; i < numeroGiriTurni; i++) {
            ...
            try {
                listaRun.add(turniService.doRunMigliorato(id,turniGiaAssergnati, turniMese, persone));
        ...
        //stampo le stats
        printStatsSuperRun(listaRun,prefixFile);
    ...
    private static void printStatsSuperRun(ArrayList<SuperRun> listaRun,String prefix) throws IOException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy_MM_dd_HHmm");
        ...
        for (int i = 0; i < giri; i++) {
            ...
            //print file
            output=statService.stampaStatistiche(listaRun.get(i).getRun1());
            Files.write(Paths.get(path+"\\"+fileName), output.getBytes(), StandardOpenOption.APPEND);
            turni=statService.stampaTurni(listaRun.get(i).getRun1());
            Files.write(Paths.get(path+"\\"+fileNameTurni), turni.getBytes(), StandardOpenOption.APPEND);

            output=statService.stampaStatistiche(listaRun.get(i).getRunMigliorato());
            Files.write(Paths.get(path+"\\"+fileName), output.getBytes(), StandardOpenOption.APPEND);
            turni=statService.stampaTurni(listaRun.get(i).getRunMigliorato());
            Files.write(Paths.get(path+"\\"+fileNameTurni), turni.getBytes(), StandardOpenOption.APPEND);
